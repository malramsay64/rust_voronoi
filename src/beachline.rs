...
impl fmt::Debug for BeachNode {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "p: {:?}, l: {:?}, r: {:?}, item: {:?}",
            self.parent, self.left_child, self.right_child, self.item
        )
    }
}
...
impl fmt::Debug for BreakPoint {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "left: {:?}, right: {:?}, halfedge: {}",
            self.left_site, self.right_site, self.halfedge
        )
    }
}
...
impl BeachLine {
    ...
    pub fn get_arc_above(&self, pt: Point) -> usize {
        if self.is_empty() {
            panic!("can't get_arc_above on empty beachline!");
        }
    ...
    pub fn successor(&self, node: usize) -> Option<usize> {
        ...
        while current_parent.is_some()
            && current_node == self.nodes[current_parent.unwrap()].right_child
        {
    ...
    pub fn predecessor(&self, node: usize) -> Option<usize> {
        ...
        let mut current_parent = self.nodes[node].parent;
        while current_parent.is_some()
            && current_node == self.nodes[current_parent.unwrap()].left_child
        {
            current_node = current_parent;
            current_parent = self.nodes[current_parent.unwrap()].parent;
        }
    ...
    pub fn get_left_arc(&self, node: usize) -> Option<usize> {
        self.predecessor(node)
            .and_then(|left| self.predecessor(left))
    }

    pub fn get_right_arc(&self, node: usize) -> Option<usize> {
        self.successor(node).and_then(|right| self.successor(right))
    }

    pub fn get_leftward_triple(&self, node: usize) -> Option<TripleSite> {
        let left_arc = self.get_left_arc(node)?;
        let left_left_arc = self.get_left_arc(left_arc)?;

        let this_site = self.get_site(node)?;
    ...
    pub fn get_rightward_triple(&self, node: usize) -> Option<TripleSite> {
        let right_arc = self.get_right_arc(node)?;
    ...
    pub fn get_site(&self, node: usize) -> Option<Point> {
